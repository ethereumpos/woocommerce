...
if ($status == "paid") {
	...
	$order->update_status( 'processing', __( 'Ethereum Payment Paid', 'ethereum-for-woocommerce' ) );
...
function wc_ethereum_gateway_init() {
	...
	class WC_Gateway_Ethereum extends WC_Payment_Gateway {
		...
		public function __construct() {
			...
			$this->method_title       = __( 'Offline', 'ethereum-for-woocommerce' );
		...
		public function create_ethereum_order($order) {
			$post_url = 'https://testapi.ethereumpos.com/order';

			$arg_data = array( 'ref_id' => $order->get_order_number(), 'amount' => $order->get_total(), 'address' => '0xoksodkokk392dk30kdk93k03', 'callback' => $this->callback );
			$data = json_encode( $arg_data );

			$args = array( 'headers' => array( 'Content-Type' => 'application/json', 'Authorization' => $this->private_key ), 'body' => $data );
		...
		public function thankyou_page() {
			...
			<iframe src="https://ethereumpos.com/payment/232101412" style="width: 100%;height: 310px;border: 0;"></iframe>
		...
		public function process_payment( $order_id ) {
			...
			$ethpos = create_ethereum_order($order);
